...
export function HomeClient({ initialUser }: HomeClientProps) {
  ...
  const [householdId, setHouseholdId] = useState<string | null>(initialUser?.user_metadata?.household_id || null)
  ...
  useEffect(() => {
    ...
    // Charger les données avec l'utilisateur initial
    const initData = async () => {
      if (initialUser) {
        const hId = initialUser.user_metadata?.household_id || null
        setHouseholdId(hId)
        await Promise.all([loadExpenses(initialUser.id, hId), loadCategories()])
      }
    }
    initData()
